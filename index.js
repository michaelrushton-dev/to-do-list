...
function placeholderGenerator() {
  ...
  if (randomNumber == randomNumberUpdated) {
    //will instead only ever return last array item if last but one item is already in randomNumberUpdated variable
    ...
    textBox.placeholder = `e.g ${textBox.placeholderItems}`;
  } else {
    randomNumberUpdated = randomNumber;
    console.log(randomNumber);
    textBox.placeholderItems = placeholderItems[randomNumberUpdated];
...
removeButton.addEventListener("click", () => {
  //select all checked checkbox items in the .check-task element of the .task-list list
  let myItems = document.querySelectorAll(
    '.task-list .check-task[type="checkbox"]:checked'
  );
  // for each item in myItems, find the closest li element and add it to parentListItem
  myItems.forEach(function (item) {
    let parentListItem = item.closest("li");
    //remove all the li tags in parentListItem
